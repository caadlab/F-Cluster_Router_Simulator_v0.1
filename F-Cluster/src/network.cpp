...
void network::network_init(int x, int y, int z, int Injection_mode, int Routing_mode, int Sa_mode, int Injection_gap, int Packet_size, int Allow_VC_Num){
    size_x = x;
    size_y = y;
    size_z = z;
    ...
    for(int i = 0; i < size_z; ++i){
		for (int j = 0; j < size_y; ++j){
            for(int k = 0; k < size_x; ++k){
				...
				node_list[i][j][k].node_init(k, j, i, &(link_list_xneg[i][j][(k + 1 == size_x) ? 0 : (k + 1)].out), &(link_list_yneg[i][(j + 1 == size_y) ? 0 : (j + 1)][k].out), &(link_list_zneg[(i + 1 == size_z) ? 0 : (i + 1)][j][k].out), &(link_list_xpos[i][j][(k == 0) ? (size_x - 1) : (k - 1)].out), &(link_list_ypos[i][(j == 0) ? (size_y - 1) : (j - 1)][k].out), &(link_list_zpos[(i == 0) ? (size_z - 1) : (i - 1)][j][k].out), Injection_mode, Routing_mode, Sa_mode, Injection_gap, Packet_size, Allow_VC_Num);
...
int network::network_max_busy_VC_num(){
	...
	for (int i = 0; i < size_z; ++i){
		for (int j = 0; j < size_y; ++j){
			for (int k = 0; k < size_x; ++k){
				for (int m = 0; m < PORT_NUM; ++m){
					int tmp = node_list[i][j][k].internal_router.VCs_list[m].num_active_VCs;
					if (tmp > ret)
						ret = tmp;
				}
			}
