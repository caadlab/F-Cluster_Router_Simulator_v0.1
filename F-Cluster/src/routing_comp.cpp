...
void routing_comp::routing_comp_init(char Cur_x, char Cur_y, char Cur_z, char Dir_in, char Mode, flit* In, bool* Out_avail, int* Xpos_credits, int* Ypos_credits, int* Zpos_credits, int* Xneg_credits, int* Yneg_credits, int* Zneg_credits){
	...
	original_flit_after_RC.valid = false;
...
// determine which direction to go on a given axis, 1-> X, 2 -> Y, 3-> Z
// return value: 1 -> positive, -1 -> negative, 0 -> both directions are fine, 2 -> error
int RC_pos_or_neg(int src, int dst, int x_or_y_or_z){
	...
	else									// when src == dst, then error out
		ret = 2;
	return ret;
}
...
int routing_comp::ROMM(int input_dir, int cx, int cy, int cz, int dx, int dy, int dz){//random select the best 
	...
	// Find the list of ports that can select from
	// First rule out those forbidden turn cases
	bool avail[6];
	for (int i = 0; i < 6; ++i)
		avail[i] = false;
	if (input_dir == DIR_ZNEG){														// Check forbideen turns for ZNEG
		if (cx != dx || cy != dy)
			printf("error! in traffic from zneg, but stil has xy offset\n");
		avail[DIR_ZNEG - 1] = true;
	}
	else if (input_dir == DIR_YNEG){
		if (cx != dx)																				// Check forbidden turn for YNEG
...
//	int rand_port_id = rand() % avail_port_num;
	int rand_port_id = 255 % avail_port_num;
//	if (avail_port_num > 2 )
	//	rand_port_id = 0;

	// Locate the output direction based on the random number generator
	int counter = 0;
	for (int i = 0; i < PORT_NUM; ++i){
		if (avail[i]){
...
// XYZ and its relatives are deadlock free, thus no need to consider input direction nor check forbidden turns 
...
int routing_comp::O1TURN(int input_dir, int cx, int cy, int cz, int dx, int dy, int dz, int O1TURN_id, int* init_id){
	...
	//ZXY 4
	//ZYX 5
	if (cx == dx && cy == dy && cz == dz)				// eject when reach the destination
		return DIR_EJECT;

	// Newly injected packets need to select from XYZ, XZY, YXZ, etc.......
	if (O1TURN_id == -1){
		...
		int rand_path_id = 255 % avail_path_num;
		//	if (avail_port_num > 2 )
		//	rand_port_id = 0;
		int counter = 0;
		int selected_path;
	...
	// If packet has already been assigned a routing algorithm, then just follow
	else{
		if (O1TURN_id == 0)
			return DOR_XYZ(cx, cy, cz, dx, dy, dz);
		else if (O1TURN_id == 1)
...
int routing_comp::RCA(int input_dir, int cx, int cy, int cz, int dx, int dy, int dz){
	...
	if (input_dir == DIR_ZNEG){
		if (cx != dx || cy != dy)
			printf("error! in traffic from zneg, but stil has xy offset\n");		// The error message just for debugging use, the routing algorithm won't let this happen in general
		avail[DIR_ZNEG - 1] = true;																// If cx!=dx || cy!=dy, DIR_ZENG shouldn't be available
	}
	else if (input_dir == DIR_YNEG){
		if (cx != dx)
			printf("error! in traffic from yneg, but still has x offset\n");
	...
	//debugging info
	if (ret == 0) {
		printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!the error packet info: source (%d, %d, %d), destination (%d, %d, %d), input direction: %d\n", cx, cy, cz, dx, dy, dz, input_dir);
		printf("	\t\t\t\tcredits(%d, %d, %d, %d, %d, %d), avail(%d, %d, %d, %d, %d, %d)\n", credits[0], credits[1], credits[2], credits[3], credits[4], credits[5], avail[0], avail[1], avail[2], avail[3], avail[4], avail[5]);
	}
...
int routing_comp::RLB(int input_dir, int cx, int cy, int cz, int dx, int dy, int dz) {
	...
	if (cx != dx) {
		if (input_dir != DIR_XPOS && input_dir != DIR_XNEG) {
			...
			return input_dir > 3 ? input_dir - 3 : input_dir + 3;										// Send out in the same direction as it received
		}
	}
	else if (cy != dy) {
...
void routing_comp::produce(){
	...
	// determine if the current packet is ejection
	...
	if (out_avail_latch) {
		...
		in_avail = true;						// this actually not used in router.app
		out = flit_after_RC;
	}
	else {
	...
	// When enters prefetch mode, the original input buffer is already assign the current packet header into the RC.in_latch, but this flit is not stored in input buffer anymore, thus need to store this packet header before enter prefetch
	// (And restore the packet header in the RC.in_latch after prefetch is done) <-- this is not needed in the current implementation. When prefetch occurs, the header flit that is stuck will be put back onto the top of the input buffer
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// When enters prefetch mode
	if (input_prefetch_enable && !prev_input_prefetch_enable) {
		// store the current flit_after_RC for future restore (this is not used in the current implementation)
		original_flit_after_RC = flit_after_RC;
		// set output as invalid so the VC input will take the new input, which is the prefetched head flit
		out.valid = false;
	}

	//////////////////////////////////////////////////////////////////////////////////////////
	// in the current implementation, there's nothing to be done for recovery from prefetching
	//////////////////////////////////////////////////////////////////////////////////////////
	
/*
	// Retore the output when prefetch is done
	if (prev_input_prefetch_enable && !input_prefetch_enable) {
		out = original_flit_after_RC;
	}
*/
	// record the previous input_prefetch
