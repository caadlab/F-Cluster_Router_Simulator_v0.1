...
void node::node_init(int Cur_x, int Cur_y, int Cur_z, flit* In_xpos, flit* In_ypos, flit* In_zpos, flit* In_xneg, flit* In_yneg, flit* In_zneg, int Injection_mode, int Routing_mode, int Sa_mode, int Injection_gap, int Packet_size, int Allow_VC_Num){
    cur_x = Cur_x;
    cur_y = Cur_y;
    cur_z = Cur_z;
...
//        inject_latch_ptrs[i] = &(app_core.inject[i]);
	...
	internal_router.router_init(cur_x, cur_y, cur_z, SA_mode, routing_mode, injection_mode, in_latch_ptrs, inject_latch_ptrs, Injection_gap, Packet_size, allow_vc_num);
