...
*/
				// Node take input from xneg, yneg, zneg,
				// xpos, ypos, zpos input all connect to void flits, in such way, those ports will always be invalid
				node_list[i][j][k].node_init(k, j, i, &(void_flit), &(void_flit), &(void_flit), &(link_list_xpos[i][j][(k == 0) ? (size_x - 1) : (k - 1)].out), &(link_list_ypos[i][(j == 0) ? (size_y - 1) : (j - 1)][k].out), &(link_list_zpos[(i == 0) ? (size_z - 1) : (i - 1)][j][k].out), Injection_mode, Routing_mode, Sa_mode, Injection_gap, Packet_size);
