...
int simulation_timeout = 20000;				// the maximum number of cycles that will run for the simulator


char comp_inject_dir(int src_x, int src_y, int src_z, int dst_x, int dst_y, int dst_z) {
	//compute which direction to inject
	char ret;
	if (dst_x == src_x && dst_y == src_y && dst_z == src_z) {
		ret = DIR_EJECT;
	}
	else if (dst_x != src_x) {
		ret = DIR_XPOS;
        ...
        for(int j = 0; j < ZSIZE; ++j){
            for(int k = 0; k < YSIZE; ++k){
                for(int m = 0; m < XSIZE; ++m){
                    ...
                    for(int n = 0; n < 2 * pattern_size; n = n + 2){
                        pattern[i][j][k][m][n].valid = true;
						total_packet_sent = total_packet_sent + 2;
                        pattern[i][j][k][m][n].src_x = m;
                        pattern[i][j][k][m][n].src_y = k;
                        pattern[i][j][k][m][n].src_z = j;
                        pattern[i][j][k][m][n].id = n;
						pattern[i][j][k][m][n+1].src_x = m;
						pattern[i][j][k][m][n+1].src_y = k;
						pattern[i][j][k][m][n+1].src_z = j;
						pattern[i][j][k][m][n+1].id = n+1;
                        if(i == 0){
                            pattern[i][j][k][m][n].dst_x = m + 1 >= XSIZE ? 0 : m + 1;
                            pattern[i][j][k][m][n].dst_y = k;
                            pattern[i][j][k][m][n].dst_z = j;
							pattern[i][j][k][m][n+1].dst_x = m != 0 ? m-1 : XSIZE - 1;
							pattern[i][j][k][m][n+1].dst_y = k;
							pattern[i][j][k][m][n+1].dst_z = j;
                        }
                        else if(i == 1){
                            pattern[i][j][k][m][n].dst_x = m;
                            pattern[i][j][k][m][n].dst_y = k + 1 >= YSIZE ? 0 : k + 1;
                            pattern[i][j][k][m][n].dst_z = j;
							pattern[i][j][k][m][n+1].dst_x = m;
							pattern[i][j][k][m][n+1].dst_y = k != 0 ? k - 1 : YSIZE - 1;
							pattern[i][j][k][m][n+1].dst_z = j;
                        }
                        else if(i == 2){
                            pattern[i][j][k][m][n].dst_x = m;
							...
							pattern[i][j][k][m][n+1].dst_x = m;
							pattern[i][j][k][m][n+1].dst_y = k;
							pattern[i][j][k][m][n+1].dst_z = j != 0 ? j - 1 : ZSIZE - 1;
                        }
                        pattern[i][j][k][m][n].mahattan_dist = 1;
                        pattern[i][j][k][m][n].sent = false;
                        pattern[i][j][k][m][n].rcvd = false;
                        pattern[i][j][k][m][n].payload = n;
						pattern[i][j][k][m][n+1].mahattan_dist = 1;
						pattern[i][j][k][m][n+1].sent = false;
						pattern[i][j][k][m][n+1].rcvd = false;
						pattern[i][j][k][m][n+1].payload = n+1;
...
int gen_pattern_three_hop_diagonal(int pattern_size){
		...
		for (int j = 0; j < ZSIZE; ++j) {
			for (int k = 0; k < YSIZE; ++k) {
				for (int m = 0; m < XSIZE; ++m) {
					...
					if (!(pattern[i][j][k][m] = (packet*)malloc(8 * pattern_size * sizeof(packet)))) {
						printf("error when allocating space for pattern\n");
						exit(-1);
					}
				}
	...
	for (int j = 0; j < ZSIZE; ++j) {
		for (int k = 0; k < YSIZE; ++k) {
			for (int m = 0; m < XSIZE; ++m) {
				for (int i = 0; i < PORT_NUM; ++i) {		// can only inject on XPOS direction, all 8 packets
					for (int n = 0; n < 8 * pattern_size; ++n){
						pattern[i][j][k][m][n].src_x = m;
						pattern[i][j][k][m][n].src_y = k;
						pattern[i][j][k][m][n].src_z = j;
						pattern[i][j][k][m][n].id = n;
						if (i == 0){
							global_injection_packet_size[i][j][k][m]++;
							pattern[i][j][k][m][n].valid = true;
							total_packet_sent++;
							if (n % 8 == 0) {
								pattern[i][j][k][m][n].dst_x = m + 1 >= XSIZE ? 0 : m + 1;
...
// generate random pattern, randomly send to 1/4 of the nodes in the network
int gen_pattern_random(int pattern_size) { //each node multicast to 26 nearest neighbors
	total_packet_sent = 0;
	char cur_inject_dir;
	bool port_used[PORT_NUM];
	for (int ii = 0; ii < PORT_NUM; ++ii) {
	...
	for (int z = 0; z < ZSIZE; ++z) {
		for (int y = 0; y < YSIZE; ++y) {
			for (int x = 0; x < XSIZE; ++x) {
				...
				for (int j = 0; j < pattern_size; ++j) {
					for (int m = 0; m < XSIZE*YSIZE*ZSIZE / 4; ++m) {
...
// Point to point pattern for debugging use
// from (0,0,0) to (7,7,7) in a 8*8*8 network
int gen_pattern_point_to_point(int pattern_size) {
	total_packet_sent = 0;
	char cur_inject_dir;
	bool port_used[PORT_NUM];
	for (int ii = 0; ii < PORT_NUM; ++ii) {
	...
	for (int z = 0; z < ZSIZE; ++z) {
		for (int y = 0; y < YSIZE; ++y) {
			for (int x = 0; x < XSIZE; ++x) {
				...
				// only send from (0,0,0)
				if (z == 0) {
					int dst_z = 7;
					int dst_y = 7;
					int dst_x = 7;

					if (dst_x != x)  cur_inject_dir = DIR_XPOS;
					else if (dst_y != y) cur_inject_dir = DIR_YPOS;
					else if (dst_z != z) cur_inject_dir = DIR_ZPOS;
					else if (dst_x == x && dst_y == y && dst_z == z) cur_inject_dir = DIR_EJECT;
...
bool count_sent_and_rcvd(int *send_num, int* packet_num, int* cycle_counter){
    int cur_sent_num = 0;
    int cur_rcvd_num = 0;
    for(int i = 0; i < PORT_NUM; ++i){
		for (int j = 0; j < ZSIZE; ++j){
...
void print_stats(int* total_packet, int total_latency, float* Avg_latency, int* Worst_case_latency, std::string* Worst_case_packet_info){
	...
	char worst_case_packet_info[200] = "";
    for(int i = 0; i < PORT_NUM; ++i){
		for (int j = 0; j < ZSIZE; ++j){
            for(int k = 0; k < YSIZE; ++k){
                for(int m = 0; m < XSIZE; ++m){
...
int main(int argc, char* argv[]){
	...
	int packet_size = 1;					// always set packet size as one for hoplite, as each flit should carry routing information
	...
	pattern_path[8] = "point_to_point_debug";

	std::string selected_pattern_path = pattern_path[pattern_id];

	std::string filename = "Hoplite_" + selected_pattern_path + "_" + argv[4] + "_" + argv[6] + "_" + argv[8] + ".csv";
	...
	// generate point_to_point pattern for debugging use
	else if (pattern_id == 8) {
		port_used_num = gen_pattern_point_to_point(pattern_size);
	}
	...
	for (int routing_mode_i = 0; routing_mode_i < 1; routing_mode_i++) {
...
//					printf("%dth cycle:\n", cycle_counter);
				...
				if (count_sent_and_rcvd(&cur_sent_counter, &cur_packet_counter, &cycle_counter)){
					break;
				}
				...
				if (cycle_counter == simulation_timeout){
					printf("%d cycles has been simulated, stop\n", simulation_timeout);

					// debugging info
/*					// print out those packets that have not been send out
					printf("\n=========================================================================\n");
					printf("Send packet list (packet have been send)\n");
					int send_counter = 0;
					int rcvd_counter = 0;
					int total_packet = 0;
							...
							for (int y = 0; y < YSIZE; ++y) {
								for (int x = 0; x < XSIZE; ++x) {
									for (int pack_id = 0; pack_id < global_injection_packet_size[port][z][y][x]; ++pack_id) {
										total_packet++;
										if (pattern[port][z][y][x][pack_id].sent) {
											send_counter++;
											//printf("Pattern[%d][%d][%d][%d][%d], send counter = %d\n", port, z, y, x, pack_id, send_counter);
										}
										if (pattern[port][z][y][x][pack_id].rcvd) {
											rcvd_counter++;
											//printf("Pattern[%d][%d][%d][%d][%d], send counter = %d\n", port, z, y, x, pack_id, send_counter);
										}
									}
					...
					printf("Total number of packet is %d, total injected packet is %d, received packet is %d\n", total_packet, send_counter, rcvd_counter);
					printf("=========================================================================\n\n");
*/
